...
function initLogin() {
  // Check if this is an upload page request (no login needed)
  if (window.location.hash.startsWith('#upload/')) {
    var pcId = parseInt(window.location.hash.replace('#upload/', ''));
    showUploadPage(pcId);
    return;
  }
...
// ══ NAV ══
function goHome() {
  // Si on est sur la page upload (hash), retourner à la page principale
  window.location.hash = '';
  // Sinon aller sur l'onglet diagnostic
  showTab('diag');
  window.scrollTo(0, 0);
}
